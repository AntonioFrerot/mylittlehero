...
type RevertSubscriptionModalProps = {
  open: boolean;
  pending?: boolean;
  onConfirm: () => void;
  onClose: () => void;
...
export function RevertSubscriptionModal({
  open,
  pending = false,
  onConfirm,
  onClose,
}: RevertSubscriptionModalProps) {
  const { t } = useLocale();
  ...
  return (
    ...
    >
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="revert-subscription-modal-title"
      ...
      >
        ...
        <button
          ...
          aria-label={t("space.revertSubscriptionModalDismiss")}
          disabled={pending}
          onClick={onClose}
        >
        ...
        <div className="tickets-required-modal__inner cancel-subscription-modal__inner">
          <p className="tickets-required-modal__eyebrow">
            {t("space.revertSubscriptionModalEyebrow")}
          </p>
          <h2
            id="revert-subscription-modal-title"
            className="cancel-subscription-modal__title"
          >
            {t("space.revertSubscriptionModalTitle")}
          </h2>
          <p className="cancel-subscription-modal__lead">
            {t("space.revertSubscriptionModalLead")}
          </p>
          ...
          <div className="cancel-subscription-modal__actions">
            <Button
              ...
              onClick={onClose}
            >
              {t("space.revertSubscriptionModalDismiss")}
            </Button>
            <Button
              type="button"
              variant="primary"
            ...
            >
              {pending
                ? t("space.revertingSubscription")
                : t("space.revertSubscriptionModalConfirm")}
            </Button>
          </div>
        </div>
      </div>
